...
| `schemaType` | JSON-LD `@type` generated by the built-in markdown renderer: `Article` (default), `BlogPosting`, or `WebPage`. |

`HowTo` is supported by contract validation / `verify` / `verify --prod` (with `HowToStep` step validation), but it is not emitted by the built-in markdown collection renderer via `schemaType`.
